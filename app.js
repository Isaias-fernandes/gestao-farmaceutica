...
async function pacientes(c){
  ...
  const pacientesVisiveis=(patients||[]).filter(x=>String(x.nome||'').trim().toUpperCase()!=='[PACIENTE EXCLUÍDO]').sort((a,b)=>String(a.nome||'').localeCompare(String(b.nome||''),'pt-BR',{sensitivity:'base'}));
  renderPatients(pacientesVisiveis);
  ...
  $('#fpm').onsubmit=async e=>{e.preventDefault();const o=Object.fromEntries(new FormData(e.target));const med=(meds||[]).find(m=>m.id===o.medication_id);const unidade=String(med?.unidade||'').trim().toLowerCase();const indivisivel=['comprimido','comprimidos','cápsula','capsula','cápsulas','capsulas','frasco','frascos','ampola','ampolas','unidade','unidades'].includes(unidade);let quantidade=+o.quantidade_mensal;if(!Number.isFinite(quantidade)||quantidade<0)return alert('Informe uma quantidade mensal válida.');if(indivisivel)quantidade=Math.ceil(quantidade);const payload={patient_id:o.patient_id,medication_id:o.medication_id,posologia:o.posologia||null,quantidade_mensal:quantidade,observacoes:o.observacoes||null,ativo:true,created_by:profile.id,updated_at:new Date().toISOString()};const {error}=await sb.from('patient_medications').upsert(payload,{onConflict:'patient_id,medication_id'});if(error)alert(error.message);else page('pacientes')};
  $('#buscaP').oninput=e=>{const q=e.target.value.toLowerCase();renderPatients(pacientesVisiveis.filter(x=>(x.nome||'').toLowerCase().includes(q)||(x.cpf||'').includes(q)||(x.cns||'').includes(q)))};
}
...
async function medicamentos(c){
  ...
  document.querySelectorAll('.deactivateMed').forEach(b=>b.onclick=async()=>{const x=rows.find(m=>m.id===b.dataset.id);if(!x)return;if(!confirm(`Excluir ${x.nome} do uso operacional?\n\nEle sairá automaticamente da lista dos pacientes e dos novos pedidos. O histórico de dispensações será preservado e a exclusão entrará no relatório com o motivo Alto custo.`))return;b.disabled=true;const oldText=b.textContent;b.textContent='Excluindo...';try{const {data,error}=await sb.rpc('excluir_medicamento_permanente',{p_medication_id:x.id,p_motivo:'Alto custo'});if(error)throw error;alert(`Medicamento ${data?.nome||x.nome} excluído do uso operacional. ${data?.vinculos_pacientes_desativados||0} vínculo(s) com paciente(s) foram retirados. Histórico preservado.`);await page('medicamentos')}catch(e){alert('Não foi possível excluir: '+(e?.message||String(e)))}finally{b.disabled=false;b.textContent=oldText}});
...
async function equipe(c){
  const [{data:users,error},{whatsapp_group_url,whatsapp_group_name}]=await Promise.all([
    sb.from('profiles').select('id,nome,telefone,role,ativo').in('role',['admin','gestor','atendente']).eq('ativo',true).order('nome'),
    getAppSettings()
  ]);
  if(error) throw error;
  const rows=(users||[]).map(u=>{
    const n=(u.telefone||'').replace(/\D/g,'');
    const wa=n?`<a class="btn whatsapp" target="_blank" rel="noopener" href="https://wa.me/${n}">Conversar</a>`:'Sem telefone';
    return `<tr><td>${esc(u.nome)}</td><td>${esc(u.telefone||'')}</td><td>${esc(u.role)}</td><td>${wa}</td></tr>`;
  }).join('');
...
async function usuarios(c){
  ...
  const [{data,error},{whatsapp_group_url,whatsapp_group_name}]=await Promise.all([
    sb.from('profiles').select('*').order('created_at'),
    getAppSettings()
  ]);
  if(error) throw error;
  c.innerHTML=`<div class="card"><h2>Grupo de WhatsApp dos gestores</h2><form id="fw"><label>Nome do grupo</label><input name="name" value="${esc(whatsapp_group_name||'Gestores - Gestão Farmacêutica')}"><label>Link de convite do grupo</label><input name="url" type="url" placeholder="https://chat.whatsapp.com/..." value="${esc(whatsapp_group_url||'')}"><p class="small">Crie o grupo no WhatsApp uma única vez, copie o link de convite e salve aqui. Após a aprovação, cada gestor verá automaticamente o botão para entrar.</p><button>Salvar grupo</button></form></div><div class="card"><h2>Usuários</h2><table><tr><th>Nome</th><th>WhatsApp</th><th>Perfil</th><th>Ativo</th><th>Ação</th></tr>${(data||[]).map(u=>`<tr><td>${esc(u.nome)}</td><td>${esc(u.telefone||'')}</td><td><select id="r_${u.id}"><option ${u.role==='pendente'?'selected':''}>pendente</option><option ${u.role==='atendente'?'selected':''}>atendente</option><option ${u.role==='gestor'?'selected':''}>gestor</option><option ${u.role==='admin'?'selected':''}>admin</option><option ${u.role==='bloqueado'?'selected':''}>bloqueado</option></select></td><td>${u.ativo?'Sim':'Não'}</td><td><button data-id="${u.id}" class="saveRole">Salvar</button></td></tr>`).join('')}</table></div>`;
  $('#fw').onsubmit=async e=>{
    e.preventDefault();
    const o=Object.fromEntries(new FormData(e.target));
    if(o.url&&!/^https:\/\/(chat\.)?whatsapp\.com\//i.test(o.url)){
      alert('Informe um link de convite válido do WhatsApp.');return;
    }
    const {error}=await sb.from('app_settings').upsert({id:1,whatsapp_group_url:o.url||null,whatsapp_group_name:o.name||'Gestores - Gestão Farmacêutica',updated_by:profile.id});
    if(error) alert(error.message); else alert('Grupo salvo. Os usuários aprovados verão o botão automaticamente.');
  };
  document.querySelectorAll('.saveRole').forEach(b=>b.onclick=async()=>{
    const role=$(`#r_${b.dataset.id}`).value;
    const {error}=await sb.from('profiles').update({role}).eq('id',b.dataset.id);
    if(error) alert(error.message); else alert(role==='gestor'||role==='atendente'||role==='admin'?'Perfil aprovado. O botão do grupo de WhatsApp já ficará disponível para este usuário.':'Perfil atualizado.');
  });
}
