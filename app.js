...
async function pacientes(c){
  const [{data:patients,error:ep},{data:meds,error:em},{data:pm,error:el}]=await Promise.all([
    ...
    sb.from('medications').select('id,nome,dosagem,forma,unidade').eq('ativo',true).order('nome'),
    sb.from('patient_medications').select('id,patient_id,medication_id,posologia,quantidade_mensal,observacoes,ativo,medications(nome,dosagem,forma,unidade)').eq('ativo',true).order('created_at')
  ]);
  ...
  const medOptions=(meds||[]).map(x=>`<option value="${x.id}">${esc(x.nome)} ${esc(x.dosagem||'')} ${esc(x.forma||'')}</option>`).join('');
