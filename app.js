...
async function pacientes(c){
  const [{data:patients,error:ep},{data:meds,error:em},{data:pm,error:el}]=await Promise.all([
    ...
    sb.from('medications').select('id,nome,dosagem,forma,unidade,apresentacao,quantidade_embalagem').eq('ativo',true).order('nome'),
    sb.from('patient_medications').select('id,patient_id,medication_id,posologia,quantidade_mensal,observacoes,ativo,medications(nome,dosagem,forma,unidade,apresentacao,quantidade_embalagem)').eq('ativo',true).order('created_at')
  ]);
  ...
  const renderPatients=(list)=>{ $('#tblP').innerHTML=patientTable(list,pm||[]); bindPatientMedActions(); };
  const bindPatientMedActions=()=>{
  ...
  $('#fp').onsubmit=async e=>{e.preventDefault();let o=Object.fromEntries(new FormData(e.target));o.created_by=profile.id;const {error}=await sb.from('patients').insert(o);if(error)alert(error.message);else page('pacientes')};
...
async function medicamentos(c){
  ...
  c.innerHTML=`${can?`<div class="card"><h2>Novo medicamento</h2><p class="small">Cadastre o medicamento de forma padronizada. A unidade será usada no estoque, na lista dos pacientes e nos pedidos automáticos.</p><form id="fm" class="grid"><div><label>Nome</label><input name="nome" required placeholder="Ex.: Losartana"></div><div><label>Dosagem</label><input name="dosagem" placeholder="Ex.: 50 mg"></div><div><label>Forma farmacêutica</label><select name="forma" required><option value="">Selecione...</option>${formas.map(v=>`<option value="${v}">${v}</option>`).join('')}</select></div><div><label>Unidade de controle</label><select name="unidade" required><option value="">Selecione...</option>${unidades.map(v=>`<option value="${v}">${v}</option>`).join('')}</select></div><div><label>Apresentação / embalagem</label><input name="apresentacao" placeholder="Ex.: Caixa com 30 comprimidos / Frasco 100 mL"></div><div><label>Quantidade por embalagem</label><input name="quantidade_embalagem" type="number" min="0" step="0.01" placeholder="Ex.: 30"></div><div><label>Estoque mínimo</label><input name="estoque_minimo" type="number" min="0" step="0.01" value="0"></div><div><label>Estoque ideal</label><input name="estoque_ideal" type="number" min="0" step="0.01" value="0"></div><div><button>Salvar medicamento</button></div></form></div>`:''}<div class="card"><h2>Medicamentos</h2><table><tr><th>Nome</th><th>Dosagem</th><th>Forma</th><th>Unidade</th><th>Apresentação</th><th>Qtd./emb.</th><th>Mínimo</th><th>Ideal</th></tr>${(data||[]).map(x=>`<tr><td>${esc(x.nome)}</td><td>${esc(x.dosagem||'')}</td><td>${esc(x.forma||'')}</td><td>${esc(x.unidade||'')}</td><td>${esc(x.apresentacao||'')}</td><td>${x.quantidade_embalagem??''}</td><td>${x.estoque_minimo}</td><td>${x.estoque_ideal}</td></tr>`).join('')}</table></div>`;
  if(can)$('#fm').onsubmit=async e=>{e.preventDefault();let o=Object.fromEntries(new FormData(e.target));o.quantidade_embalagem=o.quantidade_embalagem?+o.quantidade_embalagem:null;o.estoque_minimo=+o.estoque_minimo||0;o.estoque_ideal=+o.estoque_ideal||0;const {error}=await sb.from('medications').insert(o);if(error)alert(error.message);else{alert('Medicamento cadastrado com sucesso.');page('medicamentos')}}
}
async function estoque(c){const {data:sum}=await sb.from('stock_summary').select('*').order('nome'); const {data:meds}=await sb.from('medications').select('id,nome,dosagem,unidade,apresentacao').eq('ativo',true).order('nome'); const can=['admin','gestor'].includes(profile.role); c.innerHTML=`${can?`<div class="card"><h2>Entrada de estoque</h2><form id="fe" class="grid"><div><label>Medicamento</label><select name="medication_id">${(meds||[]).map(m=>`<option value="${m.id}">${esc(m.nome)} ${esc(m.dosagem)}${m.apresentacao?` — ${esc(m.apresentacao)}`:''} [${esc(m.unidade||'')}]</option>`).join('')}</select></div><div><label>Quantidade</label><input name="quantidade" type="number" step="0.01" required></div><div><label>Lote</label><input name="lote"></div><div><label>Validade</label><input name="validade" type="date"></div><div><label>Fornecedor</label><input name="fornecedor"></div><div><label>Documento/NF</label><input name="documento"></div><div><button>Registrar entrada</button></div></form></div>`:''}<div class="card"><h2>Estoque atual</h2><table><tr><th>Medicamento</th><th>Dosagem</th><th>Saldo</th><th>Mínimo</th><th>Ideal</th><th>Situação</th></tr>${(sum||[]).map(x=>`<tr><td>${esc(x.nome)}</td><td>${esc(x.dosagem)}</td><td>${x.estoque_atual} ${esc(x.unidade)}</td><td>${x.estoque_minimo}</td><td>${x.estoque_ideal}</td><td>${+x.estoque_atual<=+x.estoque_minimo?'<b>COMPRAR</b>':'OK'}</td></tr>`).join('')}</table></div>`; if(can)$('#fe').onsubmit=async e=>{e.preventDefault();const o=Object.fromEntries(new FormData(e.target));const {error}=await sb.rpc('add_stock',{p_medication_id:o.medication_id,p_lote:o.lote||null,p_validade:o.validade||null,p_quantidade:+o.quantidade,p_fornecedor:o.fornecedor||null,p_documento:o.documento||null});if(error)alert(error.message);else page('estoque')}}
