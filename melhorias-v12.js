...
(function(){
  ...
  async function dispensacaoUnificada(c){
    const [{data:patients,error:ep},{data:meds,error:em}] = await Promise.all([
      ...
      sb.from('medications').select('id,nome,dosagem,forma,unidade,apresentacao,estoque_atual_sem_lote').eq('ativo',true).order('nome')
    ]);
    if(ep) throw ep;
    if(em) throw em;
    ...
    c.innerHTML = `
      <div class="card">
        ...
        <p class="small">Escolha se a saída será de um lote identificado ou do estoque sem lote. O saldo correspondente será baixado automaticamente.</p>
        <form id="fdNova" class="grid">
          ...
          <div><label>Origem do estoque</label><select id="dispOrigem"><option value="lote">Estoque com lote</option><option value="sem_lote">Estoque sem lote</option></select></div>
          <div id="dispLoteBox"><label>Lote</label><select id="dispLote" name="stock_lot_id" disabled><option value="">Selecione o medicamento primeiro</option></select></div>
          <div id="dispValidadeBox"><label>Validade do lote</label><input id="dispValidade" readonly></div>
          <div><label id="dispSaldoLabel">Saldo disponível</label><input id="dispSaldo" readonly></div>
    ...
    let lotes=[];
    const medSel=$('#dispMed'), origem=$('#dispOrigem'), loteSel=$('#dispLote'), validade=$('#dispValidade'), saldo=$('#dispSaldo');
    const medAtual=()=> (meds||[]).find(x=>String(x.id)===String(medSel.value));
    const atualizarDetalhes=()=>{
      if(origem.value==='sem_lote'){
        const m=medAtual();
        validade.value='';
        saldo.value=m?String(+m.estoque_atual_sem_lote||0):'';
        return;
      }
      const l=lotes.find(x=>String(x.id)===String(loteSel.value));
    ...
    const atualizarOrigem=()=>{
      ...
      $('#dispLoteBox').classList.toggle('hidden',sem);
      $('#dispValidadeBox').classList.toggle('hidden',sem);
      loteSel.disabled=sem || !lotes.length;
      $('#dispSaldoLabel').textContent=sem?'Saldo sem lote':'Saldo do lote';
      atualizarDetalhes();
    };
    medSel.onchange=async()=>{
      loteSel.disabled=true; loteSel.innerHTML='<option>Carregando...</option>'; validade.value=''; saldo.value=''; lotes=[];
      if(!medSel.value){loteSel.innerHTML='<option value="">Selecione o medicamento primeiro</option>';atualizarOrigem();return;}
      ...
      if(error){alert(error.message);loteSel.innerHTML='<option value="">Erro ao carregar lotes</option>';atualizarOrigem();return;}
      lotes=data||[];
      loteSel.innerHTML=lotes.length?'<option value="">Selecione...</option>'+lotes.map(l=>`<option value="${l.id}">${esc(l.lote||'Sem identificação')} — val. ${l.validade?new Date(l.validade+'T00:00:00').toLocaleDateString('pt-BR'):'—'} — saldo ${l.quantidade_atual}</option>`).join(''):'<option value="">Nenhum lote com saldo disponível</option>';
      atualizarOrigem();
    };
    origem.onchange=atualizarOrigem;
    ...
    $('#fdNova').onsubmit=async e=>{
      ...
      const semLote=origem.value==='sem_lote';
      const qtd=+fd.get('quantidade');
      if(!semLote && !loteSel.value) return alert('Selecione o lote ou escolha Estoque sem lote.');
      const saldoDisponivel=+saldo.value||0;
      if(qtd>saldoDisponivel) return alert(semLote?'Estoque sem lote insuficiente.':'Estoque insuficiente neste lote.');
      ...
      const {error}=await sb.rpc('dispense_stock_direto',{
        ...
        p_stock_lot_id:semLote?null:loteSel.value,
        p_quantidade:qtd,
      ...
      alert(semLote?'Dispensação registrada e estoque sem lote atualizado.':'Dispensação registrada com sucesso e saldo do lote atualizado.');
  ...
  async function historicoDispensacoes(c){
    ...
    const rows=(data||[]).flatMap(d=>(d.dispensation_items||[]).map(i=>({
      ...
      lote:i.stock_lots?.lote||'SEM LOTE',
